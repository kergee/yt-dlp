...
- `commands/login.rs` — generic webview-based login flow: `open_login_window(app, target_url)` opens a `WebviewWindowBuilder` window at the target site's own homepage (Bilibili keeps a hardcoded dedicated login page as a special case), and on window close extracts cookies from the shared webview cookie store via `cookies_for_url` and writes them out as a Netscape-format `cookies.txt` for yt-dlp to consume. Cookie domain is read per-cookie from `cookie.domain()`, not hardcoded.
